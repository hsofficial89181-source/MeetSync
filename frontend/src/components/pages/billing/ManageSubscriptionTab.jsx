...
export default function ManageSubscriptionTab({ onSelectPlan }) {
  ...
  const hasSubscription =
    ...
    subStatus !== "canceled" &&
    subStatus !== "expired";
  const trialEndsAt = subscription?.subscription?.trial_ends_at;
  ...
  async function handleAction(action, planCode) {
    ...
    try {
      if (action === "upgrade") {
        ...
        setActionSuccess(
          res.message || "Downgrade scheduled for next billing period",
        );
      } else if (action === "cancel") {
        const res = await cancelSubscription();
        setActionSuccess(
          res.message || "Subscription will be canceled at period end",
        );
  ...
  return (
    ...
    >
      {actionError && (
        <StatusBanner type="error" icon={AlertCircle}>
          {actionError}
        </StatusBanner>
      )}
      {actionSuccess && (
        <StatusBanner type="success" icon={Check}>
          {actionSuccess}
        </StatusBanner>
      )}
      ...
      {hasSubscription ? (
        ...
        >
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 13, color: "var(--text2)", marginTop: 4 }}>
                {formatCents(subscription.plan?.price_cents)}/
                {subscription.plan?.interval === "year" ? "year" : "month"} ·{" "}
                {subscription.plan?.hours_limit || "—"} hours included
              </p>
            </div>
            <span
              style={{
          ...
          <div
            style={{ height: 1, background: "var(--border)", margin: "18px 0" }}
          />
              ...
              {subscription.payment_method ? (
                <>
                  ...
                  <span style={{ fontSize: 12, color: "var(--text3)" }}>
                    ...
                    {String(subscription.payment_method.exp_year).slice(-2)}
                  </span>
                </>
              ) : (
          ...
          {cancelAtEnd && (
            ...
            >
              ...
              Subscription cancels at end of billing period — access until{" "}
          ...
          {subscription?.subscription?.pending_plan_code && (
            ...
            >
              ...
              Plan switches to{" "}
              <strong style={{ marginLeft: 4 }}>
                {subscription.subscription.pending_plan_name}
              </strong>{" "}
        ...
        >
          ...
          <p style={{ fontSize: 13, color: "var(--text2)" }}>
            Choose a plan below to start uploading meetings.
          </p>
        </div>
      )}
      ...
      {subStatus === "trial" && trialDaysLeft !== null && (
        ...
        >
          ...
          >
            <Clock size={18} style={{ color: "var(--accent2)" }} />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text2)", marginTop: 3 }}>
              {trialDaysLeft > 0
                ? `Trial expires on ${formatDate(trialEndsAt)}. Subscribe to keep your access.`
                : "Subscribe to a plan below to continue using MeetSync."}
            </div>
          </div>
          {isAdmin && trialDaysLeft > 0 && (
            <button
              className="btn btn-primary btn-sm"
              onClick={() => {
                document.querySelector('[data-plans-section]')?.scrollIntoView({ behavior: 'smooth' });
              }}
              style={{ flexShrink: 0 }}
            >
              Choose a Plan
      ...
      {subStatus === "expired" && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text2)", marginTop: 3 }}>
              Your 7-day free trial has ended. Subscribe to a plan below to continue using MeetSync.
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            >
              ...
              >
                {Math.floor(usage.used_seconds / 3600)}h{" "}
                {Math.floor((usage.used_seconds % 3600) / 60)}m used
              </span>
              <span
                style={{ fontSize: 12, color: "var(--text)", fontWeight: 600 }}
              >
                {Math.floor(usage.remaining_seconds / 3600)}h{" "}
                {Math.floor((usage.remaining_seconds % 3600) / 60)}m remaining
              </span>
            </div>
            <div
              style={{
                ...
                style={{
                  ...
                  background:
                    billingInterval === "year"
                      ...
                      : "transparent",
                  color:
                    billingInterval === "year" ? "var(--text)" : "var(--text3)",
                  boxShadow:
                    billingInterval === "year"
                      ? "0 1px 3px rgba(0,0,0,0.15)"
                      : "none",
        ...
        >
          {plans
            ...
            .map((plan) => {
              ...
              const planIdx = visiblePlans.findIndex(
                (p) => p.code === plan.code,
              );
              const currIdx = visiblePlans.findIndex(
              ...
              const features =
                plan.interval === "year"
                  ? PLAN_FEATURES_YEARLY[baseCode] || [
                      `${plan.hours_limit} hours/year`,
                    ]
                  : PLAN_FEATURES[baseCode] || [
                      `${plan.hours_limit} hours/month`,
                    ];
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <span
                      style={{
                        fontSize: 22,
                        fontWeight: 800,
                        color: isCurrent ? "var(--accent2)" : "var(--text)",
                      }}
                    >
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!hasSubscription) {
                          ...
                          setUpgradeModal({
                            planCode: plan.code,
                            planName: plan.name.replace(" (Yearly)", ""),
                          });
                        } else if (isDowngrade) {
                          setConfirmModal({
                            action: "downgrade",
      ...
      {confirmModal && (
        <ConfirmModal
          ...
          onConfirm={() =>
            handleAction(confirmModal.action, confirmModal.planCode)
          }
        />
      )}
...
function HistoryDot({ action }) {
  const cfg = {
    ...
    trial_started: { color: "#5B6AF0", bg: "rgba(91,106,240,0.12)", Icon: Clock },
    trial_expired: { color: "#6b7280", bg: "rgba(107,114,128,0.1)", Icon: Clock },
    trial_converted: { color: "#16a34a", bg: "rgba(22,163,74,0.12)", Icon: Check },
  };
  const { color, bg, Icon } = cfg[action] || {
    color: "#6b7280",
    bg: "var(--surface2)",
    Icon: Clock,
  };
...
function ConfirmModal({
  ...
  const messages = {
    ...
    cancel:
      "Your subscription will be canceled at the end of the current billing period. You keep full access until then.",
    reactivate:
      "Your subscription will continue billing normally. No changes to your billing date.",
  };
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button
            ...
            style={{
              background:
                modal.action === "cancel" ? "#dc2626" : "var(--accent2)",
              color: "#fff",
              border: "none",
              borderRadius: 8,
              padding: "8px 20px",
...
function UpgradeModal({
  ...
  useEffect(() => {
    ...
    previewUpgrade(planCode)
      ...
      .catch(() => {
        if (!cancelled)
          setPreviewError("Unable to load upgrade preview. Please try again.");
      })
      .finally(() => {
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            style={{ fontSize: 13, fontWeight: 700, color: "var(--accent2)" }}
          >
            {planName}
          </span>
        </div>
            ...
            >
              ...
              <div
                style={{
                  fontSize: 22,
                  fontWeight: 800,
                  color: hasCharge ? "var(--text)" : "#16a34a",
                }}
              >
