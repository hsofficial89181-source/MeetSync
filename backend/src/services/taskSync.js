...
async function removeTaskFromIntegrations(taskId, assigneeEmail) {
  ...
  if (!user) {
    log.warn('removeTaskFromIntegrations: user not found', { assigneeEmail });
    return;
  }
...
async function syncPendingTasksForUser(userId, provider) {
  // Find all pending task_integrations for this user+provider
  const { rows: pendingRecords } = await pool.query(
    `SELECT ti.*, t.title, t.description, t.priority, t.due_date, t.assignee_name, t.assignee_email,
     ...
     WHERE ti.user_id = $1 AND ti.provider = $2 AND ti.status = 'pending'`,
    [userId, provider]
  );
  ...
  if (provider === 'slack') {
    ...
    if (user) {
      const { rows: [tm] } = await pool.query(
        'SELECT slack_user_id FROM team_members WHERE email = $1',
  ...
  for (const record of pendingRecords) {
    ...
    try {
      const result = await syncToProvider(task, provider, integration.config, slackUserId);
